...
2. 最后一个输入框里，把面板设备上的 **Panel Layout 00 到 Panel Layout 03**
   共 4 个分片实体**按顺序全部勾选**（HA 所有实体状态硬上限 255 字符，长布局
   由蓝图切片写入、面板自动拼接；每片 250 字符，4 片约可容纳 25+ 台设备）。

生成的布局使用 `房间=实体|别名, 实体|别名; …` 紧凑格式，设备显示名沿用 HA
名称（别名由蓝图自动带上，`|`/`=`/`,` 等符号会被安全替换）。
...
按键名沿用原版键位称呼；**同步到面板的任何设备都可以绑到任何一个键**：

| 物理键 | 没绑定时短按 | 长按 |
|---|---|---|
| 灯按键 (F4) | 打开第一张灯卡片 | 进绑定页 |
| 窗帘按键 (F5) | 打开第一张窗帘卡片 | 进绑定页 |
| 音乐按键 (F6) | 打开第一张媒体卡片 | 进绑定页 |
| 空调按键 (F7) | 打开第一张空调卡片 | 进绑定页 |
| 自定义按键一~四 (F8–F11) | 进绑定页 | 进绑定页 |

**怎么绑**（两种入口任选）：
- 面板上下拉快捷面板 → **快捷键绑定**，选一个键；
- 或者**直接长按那个物理键**，立刻进入它的绑定页。

绑定页里从 **设备（同步到面板的全部设备）/ 场景 / 房间** 里选一个目标 →
**保存**。不选中直接保存 = 解除绑定（再点一次已选中的也能取消）。
