...
| 原装系统 | 本面板 |
|---|---|
| 要在面板上手工填写 HA 地址和长期访问令牌才能连上 | **不需要填任何东西**：HA 的 ESPHome 集成会自动发现面板，点一下"采纳"就完成连接 |
...
**想自动分房间（推荐，零手写）**：在 HA 里导入本仓库的蓝图
`blueprints/automation/astrion_panel_layout.yaml`（设置 → 自动化与场景 → 蓝图 →
右上角"导入蓝图"，粘贴该文件的 GitHub 链接），然后创建自动化、只选一个
"Panel Layout"实体即可——面板按你家里的房间自动分组，加了新设备也会自动更新。
