...
import com.example.astrion.utils.acceptAsync
...
class ServerImpl(private val dispatcher: CoroutineDispatcher = Dispatchers.IO) : Server {
    ...
    private fun connectClient(socket: AsynchronousSocketChannel): Flow<MessageLite> = flow {
        val client = ClientConnection(socket)
        connection.getAndUpdate { client }?.close()
        try {
            emitAll(client.readMessages())
        } catch (t: Throwable) {
            // A bad/unsupported client (e.g. an encrypted ESPHome client hitting
            // this plaintext server) must not terminate the whole accept loop.
            Timber.e(t, "Client connection error")
        } finally {
